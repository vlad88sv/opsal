...
if (mysqli_num_rows($r_ordenes) == 1)
{
    ...
    echo '<h1>Mostrando datos según última recepción del contenedor <b>'.$f['codigo_contenedor'].'</b></h1><hr />';

    $cepa_salida = ($f['dias_cepa'] ? $f['cepa_salida'].' ['.$f['dias_cepa'].' días desde salida de CEPA]' : 'Sin datos');
    ...
    echo '<div style="border: 1px solid grey;border-radius:10px; padding:15px;margin:10px;">';
        ...
        if ($f['fechatiempo_egreso'] != '')
        {
            ...
            echo '<button class="bq_eliminar_despacho" rel="'.$f['codigo_orden'].'" style="border:1px dotted red; background-color:black;color:red;margin:10px auto;padding: 10px;">Eliminar despacho</button>';
        } else {
    ...
    if (mysqli_num_rows($r_movimientos) > 0)
    {
        ...
        {
            $pos_actual = $g['x2'].'-'.$g['y2'].'-'.$g['nivel'];
            echo sprintf('<tr><td>%s</td><td>%s</td><td>%s</td></tr>', $pos_anterior, $pos_actual,$g['fechatiempo']);
            $pos_anterior = $pos_actual;
        }
        echo '<thead>';
        echo '<tr><th>Posición anterior</th><th>Posición nueva</th><th>Fecha</th></tr>';
        echo '</thead>';
        echo '</table>';
    } else {
