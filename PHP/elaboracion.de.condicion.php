...
if (!empty($_GET['ID']) && is_numeric($_GET['ID']))
{
    ...
    $c = 'SELECT `codigo_agencia`, `codigo_contenedor`, `tipo_contenedor`, `referencia_papel`, `codigo_cheque`, `fecha_ingreso` FROM `opsal_condiciones` WHERE `ID_condicion` = ' . $id_condicion;
    $r = db_consultar($c);
    $f = db_fetch($r);   
}
?>
...
{
    $DATOS = array_intersect_key($_POST,array_flip(array('ID_condicion', 'codigo_agencia', 'codigo_contenedor', 'codigo_cheque', 'referencia_papel','fecha_ingreso','tipo_contenedor')));
...
if (mysqli_num_rows($resultado) == 0)
{
    ...
    {
        $ultimos_ingresos .= sprintf('<tr><td><a href="%s">%s</a></td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>','/elaboracion.de.condicion.html?ID='.$f['ID_condicion'],$f['fecha_ingreso'],$f['nombre_operador'],$f['nombre_agencia'],$f['codigo_cheque'],$f['codigo_contenedor'],$f['tipo_contenedor'],$f['referencia_papel']);
    }
    $ultimos_ingresos .= '<thead><tr><th>Fecha elaborada</th><th>Ingresó</th><th>Agencia</th><th>Cheque</th><th>Contenedor</th><th>Tipo</th><th>EIR</th></tr></thead>';
