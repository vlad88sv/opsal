...
?>
<h1 class="opsal_titulo">Revisión de marchamos</h1>
<?php
if (isset($_POST['guardar']))
{
    $DATOS = array_intersect_key($_POST,array_flip(array('codigo_agencia', 'codigo_contenedor', 'estado_marchamos', 'codigo_cheque', 'notas','referencia_papel')));
    $DATOS['ingresado_por'] = _F_usuario_cache('codigo_usuario');
    if (db_agregar_datos('opsal_marchamos',$DATOS) > 0)
    {
        registrar('Revisión de marchamos realizada a contenedor <b>'.$_POST['codigo_contenedor'].'</b>','marchamos');
        echo '<hr /><p class="opsal_notificacion">Marchamos ingresado exitosamente.</p><hr />';
    }
}
?>
<div class="opsal_burbuja">
<form id="form_marchamos" action="/marchamos.html" method="post" autocomplete="off">
<table id="opsal_ims">
    <tbody>
        <tr>
        ...
        <tr>
            <td>Estado de marchamos</td>
            <td>
                <div id="estado_marchamos" style="text-align: center;">
                    <input type="radio" name="estado_marchamos" id="estado_marchamos_bueno" value="bueno"><label for="estado_marchamos_bueno">Bueno</label>
                    <input type="radio" name="estado_marchamos" id="estado_marchamos_malo" value="malo"><label for="estado_marchamos_malo">Malo</label>
                </div>
            </td>
        </tr>
        <tr>
...
FROM `opsal`.`opsal_marchamos` AS t1 LEFT JOIN `opsal`.`opsal_usuarios` AS t2 ON (t1.codigo_agencia = t2.codigo_usuario) LEFT JOIN `opsal`.`opsal_cheques` AS t3 USING(codigo_cheque) LEFT JOIN `opsal`.`opsal_usuarios` AS t4 ON (t1.ingresado_por = t4.codigo_usuario) 
...
if (mysqli_num_rows($resultado) == 0)
{
    ...
    {
        $ultimos_ingresos .= sprintf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>',$f['ID_marchamos'],$f['nombre_operador'],$f['nombre_agencia'],$f['nombre_cheque'],$f['codigo_contenedor'],$f['referencia_papel'],$f['estado_marchamos'],$f['fecha_ingreso'],$f['notas']);
    }
    $ultimos_ingresos .= '<thead><tr><th>ID</th><th>Ingresó</th><th>Agencia</th><th>Cheque</th><th>Contenedor</th><th>Referencia</th><th>Estado marchamos</th><th>Revisión</th><th>Notas</th></tr></thead>';
...
<script type="text/javascript">
    $( "#estado_marchamos" ).buttonset();
    
    $('#form_marchamos').submit(function () {        
        ...
        if (!$("input:radio[name='estado_marchamos']").is(":checked"))
        {
            alert ("Seleccione estado del marchamos.");
            return false;
        } 
    });
</script>