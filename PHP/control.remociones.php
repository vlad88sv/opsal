...
$c = 'SELECT CONCAT( x2,  "-", y2,  "-", nivel ) AS "Nueva posición", DATE(  `fechatiempo_ingreso` ) AS  "Ingreso", DATEDIFF( NOW( ) ,  `fechatiempo_ingreso` ) AS  "Días en patio", IF (`fechatiempo_egreso` IS NULL, "N/A" , DATE(  `fechatiempo_egreso` )) AS  "Salida", ( `arivu_ingreso` + INTERVAL 90 DAY) AS "Expiración ARIVU", DATEDIFF(  `arivu_ingreso` + INTERVAL 90 DAY, NOW( ) ) AS  "Días para exp. ARIVU", `codigo_contenedor` AS "Contenedor" FROM  `opsal_movimientos` AS t0 LEFT JOIN `opsal_ordenes` AS t1 USING(codigo_orden) LEFT JOIN  `opsal_posicion` AS t2 USING ( codigo_posicion ) WHERE t1.fechatiempo_egreso IS NOT NULL AND DATE(t1.fechatiempo_egreso) BETWEEN "'.$fecha_inicio.'" AND "'.$fecha_final.'" ORDER BY t1.codigo_contenedor ASC, t0.fechatiempo DESC';
$r = db_consultar($c);
?>
