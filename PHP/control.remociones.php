...
$c = 'SELECT t0.fechatiempo AS "Fecha", `usuario` AS "Naviera", `codigo_contenedor` AS "Contenedor", CONCAT( t2.x2,  "-", t2.y2,  "-", t0.nivel ) AS "Nueva posición", DATE(  `fechatiempo_ingreso` ) AS  "Ingreso", DATEDIFF( NOW( ) ,  `fechatiempo_ingreso` ) AS  "Días en patio", IF (`fechatiempo_egreso` IS NULL, "N/A" , DATE(  `fechatiempo_egreso` )) AS  "Salida", ( `arivu_ingreso` + INTERVAL 90 DAY) AS "Exp. ARIVU", DATEDIFF(  `arivu_ingreso` + INTERVAL 90 DAY, NOW( ) ) AS  "<acronym title=\'Días para expiración de ARIVU\'>DPEA</acronym>" FROM  `opsal_movimientos` AS t0 LEFT JOIN `opsal_ordenes` AS t1 USING(codigo_orden) LEFT JOIN  `opsal_posicion` AS t2 ON t0.`codigo_posicion` = t2.`codigo_posicion` LEFT JOIN opsal_usuarios AS t3 ON t0.cobrar_a=t3.codigo_usuario WHERE DATE(t0.fechatiempo) BETWEEN "'.$fecha_inicio.'" AND "'.$fecha_final.'" ORDER BY t1.codigo_contenedor ASC, t0.fechatiempo DESC';
$r = db_consultar($c);
?>
...
$c = 'SELECT tipo_contenedor AS "Tipo", COUNT(*) AS "Cantidad" FROM  `opsal_movimientos` AS t0 LEFT JOIN `opsal_ordenes` AS t1 USING(codigo_orden) WHERE t1.fechatiempo_egreso IS NOT NULL AND DATE(t1.fechatiempo_egreso) BETWEEN "'.$fecha_inicio.'" AND "'.$fecha_final.'" GROUP BY tipo_contenedor' ;
$r = db_consultar($c);
echo db_ui_tabla($r,'class="tabla-estandar opsal_tabla_ancha opsal_tabla_borde_oscuro"');
echo '</div>';
?>
